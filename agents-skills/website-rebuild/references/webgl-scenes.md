...
- **离线 diff 证同后，差异排查聚焦编译参数/数据链**：先用 `node diff` 证明 shader 与源站逐字一致，此后像素差异就不必再怀疑 shader 文本本身。noomo F1（全屏竖纹）即以此定案：shader 逐字相同，根因是 GLSL 版本默认值——源站默认 GLSL1、复刻误设 GLSL3 → 全部 shader 编译失败 → 空场 → NaN，一行修复级联解决三个表观 bug【noomo】。
